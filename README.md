...
🟡 **Beta Phase 1 complete** — Channel ผิด (กลุ่ม programmer + MC), pivot ไปทดสอบ target ที่ถูกใน Phase 2

- Frontend: https://your-app.vercel.app
- LIFF: https://liff.line.me/YOUR_LIFF_ID
- Backend: https://your-n8n.railway.app
- Admin: Retool (SlipScan Admin)
...
- ❌ **Tax calculation / Tax filing assistance** — OCR สลิปไม่ใช่ data source ที่เหมาะกับ pain เรื่องภาษี (สลิปไม่บอกประเภทเงินได้, ไม่บอกยอดหัก ณ ที่จ่าย) เอกสารที่เหมาะคือ 50 ทวิ ซึ่งเป็นคนละ document type
- ❌ **Slip fraud verification (Slip OK API / bank API)** — ตัดออกจาก scope ทั้งหมด ไม่ทำทั้งใน beta และหลัง beta
- ❌ **Receipt OCR (ใบเสร็จร้านค้า)** — รองรับเฉพาะสลิปโอนเงิน ไม่รวมใบเสร็จที่มี line item

### Target use case
- ร้านค้า/แม่ค้าออนไลน์ที่รับโอนเงินจากลูกค้า → ต้องการรวบรวม/ตรวจสอบยอดรับโอนเป็น CSV
- ฟรีแลนซ์รับงานจากลูกค้ารายบุคคล (ที่ระบบสรรพากรไม่ขึ้นโชว์ให้)

### Target ที่ตัดออก (จาก insight 7-8 คนใน community)
- ❌ **ฟรีแลนซ์ที่รับงานจากบริษัท** — ระบบสรรพากรขึ้นโชว์ให้อัตโนมัติแล้ว (pain ไม่แรงพอจะใช้ tool)
- ❌ **กลุ่มหางาน** (MC/PG/Staff, Video Editor, ฯลฯ) — intent mismatch + feed velocity สูง (ดู Channel Strategy)

---

## Beta Results (Phase 1)

### Programmer Communities (3 กลุ่ม)
- **Users:** 7 คน
- **Receipts ทั้งหมด:** 11 ใบ (1.5 ใบ/user)
- **Active user:** 1 คน — 8 ใบ
- **ส่วนใหญ่:** ลองครั้งเดียวแล้วหยุด

### Key Feedback
1. **2 คนใน 7 ตีความผิดว่าเป็น "เครื่องตรวจสลิปปลอม"** ทั้งที่ทาง product ไม่ได้เคลม
   - คำว่า "OCR สลิป" ในหัวคนไทย = ตรวจโอน (fraud detection)
   - SlipScan ไม่ทำ fraud detection (out of scope) → ลดความคาดหวังที่ผิดได้ด้วยการเปลี่ยน positioning ในโพสต์
2. **คู่แข่งใน niche เดียวกัน: "ป้านวล"** (LINE OA จดรายรับรายจ่าย) — ไม่มี OCR ต้องพิมพ์เอง
3. **QR + webhook bypass ปัญหา "เงินเข้าจริงไหม" ได้แล้ว** = ตลาด "ตรวจโอน" มีทางแก้ที่ดีกว่า OCR

### MC/Staff/PG Communities
- **Users:** 0
- **Receipts:** 0
- **สาเหตุ:** Intent mismatch (ดู Channel Strategy ด้านล่าง)

---

## Channel Strategy (สิ่งที่เรียนรู้แล้ว)

### ❌ Bad fit channels (ห้ามใช้ซ้ำ)
**กลุ่ม Facebook ประเภท "รับสมัครงาน/หางาน" ทั้งหมด** ไม่ว่าจะอาชีพอะไร
- Fastwork, Video Editor, MC/Staff/PG — ทุกประเภทกลุ่มหางาน
- **เหตุผล 1:** Intent mismatch — คนเปิดกลุ่มมาเพื่อ**หา income** ไม่ใช่ **จัดการ income**
- **เหตุผล 2:** Feed velocity สูง — โพสต์งานเข้าทุก 5-10 นาที โพสต์อื่นถูกกลบใน 1-2 ชม.
- **เหตุผล 3:** Algorithm ไม่ดันโพสต์ที่ engagement ต่ำใน 1-2 ชม.แรก

### ✅ Channels ที่ควรลอง (ยังไม่ได้ทดสอบ)
- **กลุ่มขายของออนไลน์ขนาดเล็ก** — รับเงินจากลูกค้ารายบุคคล (ตรง pain)
- **กลุ่ม community ฟรีแลนซ์ที่ไม่ใช่กลุ่มหางาน** — เช่น กลุ่มสนทนาฟรีแลนซ์, กลุ่มทำงานออนไลน์
- **Pantip ห้องการเงิน/ทำมาหากิน** — ตอบกระทู้ที่คนถาม "จดรายรับยังไง"
- **กลุ่มจัดการเงินส่วนตัว / ภาษีฟรีแลนซ์** (ที่เป็น end-user ไม่ใช่นักบัญชี)

### Posting approach ที่เรียนรู้แล้ว
- ❌ **อย่าโพสต์เป็น product launch** ("มาลองระบบ X") — ดูเป็น sales spam
- ✅ **โพสต์เป็นคำถาม discovery** ("ทุกคนจดรายรับยังไงครับ?") — ได้ทั้ง insight + คนสนใจ product แบบ pull
- ✅ **เปิดเผย pain ตัวเองก่อน** เพื่อลด barrier ให้คนตอบ
- ✅ **แอบบอก product ในตอนท้าย** ไม่ใช่หัวเรื่องหลัก

### Insight goldmine (เก็บไว้แล้ว — ดู `/mnt/project/1000045002-008.jpg`)
จากการคุยกับ 7-8 คนในกลุ่ม freelance:
- ฟรีแลนซ์รับงานบริษัท: สรรพากรขึ้นโชว์ให้ → pain ไม่แรง
- ฟรีแลนซ์รับงานบุคคล: ไม่มีอะไรช่วย → ต้องจำเอง 100% → **pain แรงสุด**
- ไม่มีใครพูดถึง app/tool — ตลาดเปิด แต่ pain ไม่ถึงขั้น "ลุกมาหา tool"
